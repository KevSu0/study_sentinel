...
export function usePlanData(selectedDate: Date) {
  ...
  const { isLoaded, tasks, routines, allCompletedWork } = state;
  ...
  const { upcomingItems, overdueTasks, completedForDay } = useMemo(() => {
    ...
    const completedActivities = (allCompletedWork ?? []).map((work, index) => ({
      ...work,
      id: `${work.timestamp}-${index}`,
    }));

    const selectedDayRoutines = routines.filter((r) => r.days.includes(selectedDate.getDay()));
    
    const completedActivityForDay = completedActivities
        .filter(activity => activity && typeof activity.timestamp === 'string' && new Date(activity.timestamp).toISOString().startsWith(selectedDateStr));

    const completedRoutineIds = new Set(
      completedActivityForDay
        .filter(a => a.type === 'routine' && !a.isUndone)
        .map(a => a.id)
    );
    const completedTaskIds = new Set(
      completedActivityForDay
        .filter(a => a.type === 'task' && !a.isUndone)
        .map(a => a.id)
    );
  ...
  }, [isLoaded, routines, tasks, selectedDate, selectedDateStr, todayStr, allCompletedWork]);
