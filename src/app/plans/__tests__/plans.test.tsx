...
// We are not mocking AddItemDialog anymore to allow for integration testing.
// jest.mock(
//   '@/components/tasks/add-task-dialog',
//   () => ({
//     __esModule: true,
//     AddItemDialog: jest.fn().mockImplementation(props => <div {...props} />),
//   })
// );
...
const mockTasks: StudyTask[] = [
  { id: 't1', shortId: 's1', title: 'Upcoming Task', date: todayStr, status: 'todo', time: '09:00', priority: 'medium', duration: 30, timerType: 'countdown', description: '', points: 10 },
  { id: 't2', shortId: 's2', title: 'Overdue Task', date: yesterdayStr, status: 'todo', time: '10:00', priority: 'high', duration: 60, timerType: 'countdown', description: '', points: 20 },
  { id: 't3', shortId: 's3', title: 'Completed Task', date: todayStr, status: 'completed', time: '11:00', priority: 'low', duration: 45, timerType: 'countdown', description: '', points: 15 },
  { id: 't4', shortId: 's4', title: 'In Progress Task', date: todayStr, status: 'in_progress', time: '12:00', priority: 'medium', duration: 20, timerType: 'infinity', description: '', points: 0 },
];

const mockRoutines: Routine[] = [
  { id: 'r1', shortId: 'r1s', title: 'Upcoming Routine', days: [today.getDay()], startTime: '10:00', endTime: '11:00', priority: 'medium', description: 'A routine' },
  { id: 'r2', shortId: 'r2s', title: 'Completed Routine', days: [today.getDay()], startTime: '11:00', endTime: '12:00', priority: 'low', description: 'Another routine' },
];
...
describe('PlansPage', () => {
  ...
  it('should handle editing an overdue task', async () => {
    ...
    expect(await within(dialog).findByText('Edit Task')).toBeInTheDocument();
  });
  ...
  it('should show a global empty state when no items exist for the day', async () => {
    ...
    const dialog = await screen.findByRole('dialog');
    expect(dialog).toBeInTheDocument();
    expect(await within(dialog).findByText('Add New Item')).toBeInTheDocument();
  });
  ...
  describe('Completed Items Widget', () => {
    ...
    it('should handle hard undo for a task', async () => {
        ...
        const hardUndoButton = await screen.findByRole('menuitem', { name: /hard undo/i });
  ...
  describe('Offline User Flows', () => {
    it('should complete an upcoming task by clicking the checkbox', async () => {
      const user = userEvent.setup();
      setup('card');
      const upcomingTask = await screen.findByText('Upcoming Task');
      const taskItem = upcomingTask.closest('[data-testid="plan-item-card"]') as HTMLElement;
    ...
    it('should create a new task when offline', async () => {
      const user = userEvent.setup();
      setup();
      const fab = (await screen.findAllByRole('button', { name: 'Add New Item' })).find(b => b.classList.contains('fixed'));
      ...
      await user.type(within(dialog).getByLabelText('Task Title'), 'New Offline Task');
    ...
    it('should edit an existing task when offline', async () => {
      const user = userEvent.setup();
      setup();
      const taskTitle = await screen.findByText('Upcoming Task');
      const taskCard = taskTitle.closest('[data-testid="plan-item-card"]') as HTMLElement;
      ...
      const dialog = await screen.findByRole('dialog');
      expect(dialog).toBeInTheDocument();
      expect(within(dialog).getByLabelText('Task Title')).toHaveValue('Upcoming Task');

      await user.clear(within(dialog).getByLabelText('Task Title'));
      await user.type(within(dialog).getByLabelText('Task Title'), 'Updated Offline Task');
