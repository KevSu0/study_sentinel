...
jest.mock('@/components/badges/badge-dialog', () => ({
  BadgeDialog: jest.fn(({ isOpen, onOpenChange, onAddBadge, onUpdateBadge, badgeToEdit }) => {
    ...
    const handleSave = () => {
      if (badgeToEdit) {
        onUpdateBadge(badgeToEdit.id, { description: 'Updated description' });
      } else {
...
describe('ManageBadgesPage', () => {
  ...
  it('should call updateBadge when the toggle switch is clicked', () => {
    ...
    if (!badge) throw new Error('Test setup error: badge not found');
    const expectedBadge = { ...badge, isEnabled: false };
    expect(mockUpdateBadge).toHaveBeenCalledWith(expectedBadge);
  });
  ...
  describe('BadgeDialog in Edit Mode', () => {
    it('should open with pre-filled data and submit updated data', async () => {
      ...
      await waitFor(() => {
        const badge = mockBadges.find(b => b.id === '3');
        if (!badge) throw new Error('Test setup error: badge not found');
        const expectedBadge = { ...badge, description: 'Updated description' };
        expect(mockUpdateBadge).toHaveBeenCalledWith(expectedBadge);
      });
    });
  });
