...
export interface GoldenDatasetRecord {
  id: string;
  timestamp: number;
  type: 'study_session_created' | 'study_session_updated' | 'task_created' | 'task_completed' | 'badge_earned';
  data: any;
...
export const GOLDEN_DATASET: GoldenDatasetRecord[] = [
...
export const COMBINED_EXPECTED_METRICS: ExpectedMetrics = {
...
export function validateGoldenDataset(): boolean {
...
export function calculateExpectedMetrics(records: GoldenDatasetRecord[]): ExpectedMetrics {
...
export function validateInvariants(metrics: ExpectedMetrics): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
