...
describe('CompletedPlanListItem', () => {
  ...
  describe('when isUndone is true', () => {
    ...
    it('disables the "Undo Completion" menu item', async () => {
  ...
  describe('user interactions', () => {
    ...
    const onHardUndo = jest.fn();

    beforeEach(() => {
      onUndo.mockClear();
      onHardUndo.mockClear();
      render(
        <CompletedPlanListItem
          item={mockTaskItem}
          isUndone={false}
          onUndo={onUndo}
          onHardUndo={onHardUndo}
        />
      );
    });

    it('calls onUndo when "Undo Completion" is clicked', async () => {
      await userEvent.click(screen.getByRole('button', {name: /open menu/i}));
      await userEvent.click(
        screen.getByRole('menuitem', {name: /undo completion/i})
      );
      expect(onUndo).toHaveBeenCalledTimes(1);
    });

    it('calls onHardUndo when "Hard Undo" is clicked', async () => {
      await userEvent.click(screen.getByRole('button', {name: /open menu/i}));
      await userEvent.click(
        screen.getByRole('menuitem', {name: /hard undo/i})
      );
      expect(onHardUndo).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it('does not throw error if callbacks are not provided', async () => {
    render(<CompletedPlanListItem item={mockTaskItem} isUndone={false} />);
    await userEvent.click(screen.getByRole('button', {name: /open menu/i}));
    const undoButton = screen.getByRole('menuitem', {name: /undo completion/i});
    const hardUndoButton = screen.getByRole('menuitem', {name: /hard undo/i});
