...
beforeAll(() => {
  consoleWarnSpy = jest.spyOn(console, 'warn').mockImplementation((message?: unknown, ...args: unknown[]) => {
    ...
    if (text.includes(ALERT_DIALOG_A11Y_WARNING)) {
      // Temporary: the Radix adapter omits aria-describedby; tracked in PWA governance backlog.
      return;
    }
...
const renderComponent = (props: any) => {
  return render(<PlanItemCard {...props} />);
};
...
describe('PlanItemCard', () => {
  ...
  describe('Completed Routine (`type: "completed_routine"`)', () => {
    const props = {
        ...
        onHardUndoCompleteRoutine: jest.fn(),
    };
    ...
    it('renders with line-through style and undo dropdown', async () => {
        ...
        expect(screen.getByText('Delete Log Permanently')).toBeInTheDocument();
    });

    it('calls onUndoCompleteRoutine from dropdown', async () => {
        renderComponent(props);
        await userEvent.click(screen.getByRole('button', {name: /undo completion/i}));
        await userEvent.click(screen.getByText('Undo Completion'));
        expect(props.onUndoCompleteRoutine).toHaveBeenCalledWith(mockCompletedLog.id);
    });

    it('calls onHardUndoCompleteRoutine from dropdown', async () => {
        renderComponent(props);
        await userEvent.click(screen.getByRole('button', {name: /undo completion/i}));
        await userEvent.click(screen.getByText('Delete Log Permanently'));
        expect(props.onHardUndoCompleteRoutine).toHaveBeenCalledWith(mockCompletedLog.id);
    });
  });
