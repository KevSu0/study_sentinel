...
const ManualLogDialog = lazy(() => import('../tasks/manual-log-dialog').then(m => ({ default: m.ManualLogDialog })));
...
interface PlanItemCardProps {
  ...
  onHardUndoCompleteRoutine?: (logId: string) => void;
}
...
export const PlanItemCard = React.memo(function PlanItemCard({
  ...
  const handleUndoCompleteTask = React.useCallback(() => {
             ...
             {item.type === 'completed_routine' && (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button size="icon" variant="ghost" className="h-8 w-8 text-green-600" aria-label="Undo completion">
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem onSelect={() => onHardUndoCompleteRoutine?.(item.data.id)} className="text-destructive">
      ...
      {isLogDialogOpen && (
        <Suspense fallback={<Skeleton className="h-64 w-full" />}>
          <ManualLogDialog
            ...
            item={item.data as StudyTask | Routine}
          />
        </Suspense>
      )}
    </>
