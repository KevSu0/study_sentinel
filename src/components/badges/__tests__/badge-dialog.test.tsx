...
describe('BadgeDialog', () => {
  ...
  it('calls onAddBadge with correct data on valid submission in "create" mode', async () => {
    ...
    await waitFor(() => {
      expect(defaultProps.onAddBadge).toHaveBeenCalledWith({
        ...
        icon: 'Trophy',
        conditions: [{ type: 'TASKS_COMPLETED', target: 10, timeframe: 'TOTAL' }],
        category: 'overall',
        requiredCount: 10,
        isCustom: true,
        isEnabled: true,
    ...
    await waitFor(() => {
      expect(defaultProps.onUpdateBadge).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          conditions: [{ type: 'TASKS_COMPLETED', target: 15, timeframe: 'TOTAL' }],
        })
      );
    });
  });
  ...
  describe('getTargetLabel', () => {
    it('shows "Target (Days)" for DAY_STREAK', async () => {
      const user = userEvent.setup();
      render(<BadgeDialog {...defaultProps} />);
      const selectTrigger = screen.getByRole('combobox');
      await user.click(selectTrigger);
      const option = await screen.findByRole('option', { name: 'Study Day Streak' });
      await user.click(option);
      expect(await screen.findByLabelText('Target (Days)')).toBeInTheDocument();
    });

    it('shows "Target (Count)" for TASKS_COMPLETED', async () => {
        const user = userEvent.setup();
        render(<BadgeDialog {...defaultProps} />);
        const selectTrigger = screen.getByRole('combobox');
        await user.click(selectTrigger);
        const option = await screen.findByRole('option', { name: 'Tasks Completed' });
        await user.click(option);
        expect(await screen.findByLabelText('Target (Count)')).toBeInTheDocument();
    });

    it('shows "Duration (Minutes)" for TOTAL_STUDY_TIME', async () => {
        const user = userEvent.setup();
        render(<BadgeDialog {...defaultProps} />);
        const selectTrigger = screen.getByRole('combobox');
        await user.click(selectTrigger);
        const option = await screen.findByRole('option', { name: 'Total Study Time' });
        await user.click(option);
        expect(await screen.findByText('Duration (Minutes)')).toBeInTheDocument();
    });
  });
});