...
describe('ActivityItem', () => {
  ...
  it('calls onUndo and onHardUndo from the dropdown menu', async () => {
    const user = userEvent.setup();
    const taskCompleteItem: ActivityFeedItem = {
      ...baseItem,
      type: 'TASK_COMPLETE',
    ...
    await user.click(moreButton);
    const hardUndoButton = await screen.findByRole('menuitem', { name: /Hard Undo/ });
    await user.click(hardUndoButton);
    expect(defaultProps.onHardUndo).toHaveBeenCalledTimes(1);
  });

  it('does not crash if onUndo/onHardUndo are not provided', async () => {
    const user = userEvent.setup();
    const taskCompleteItem: ActivityFeedItem = {
      ...baseItem,
      type: 'TASK_COMPLETE',
