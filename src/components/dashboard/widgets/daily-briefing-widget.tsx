...
const motivationalQuotes = [
  ...
  { quote: 'The expert in anything was once a beginner. Keep learning.', author: 'Helen Hayes' },
...
export const DailyBriefingWidget = ({ getPreviousDayLogs, profile }: any) => {
  const [dailySummary, setDailySummary] = useState<{ evaluation: string; motivationalParagraph: string } | null>(null);
    ...
    return (
      <Card className="bg-primary/5">
        ...
        <CardContent className="space-y-4">
          <div>
            <h3 className="font-semibold text-primary/90">Yesterday's Evaluation</h3>
            <p className="text-sm text-muted-foreground italic">{dailySummary.evaluation}</p>
          </div>
          <div>
            <h3 className="font-semibold text-primary/90">Today's Motivation</h3>
            <p className="text-sm text-muted-foreground italic">{dailySummary.motivationalParagraph}</p>
          </div>
        </CardContent>
      </Card>
    );
    ...
    <Card className="bg-primary/5">
      <CardContent className="p-4 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-right font-medium text-muted-foreground mt-1">- {dailyQuote.author}</p>
        </div>
      </CardContent>
    </Card>
  );
