...
interface RealProductivityWidgetProps {
  data: ProductivityDataPoint[];
  isLoaded: boolean;
}

export function RealProductivityWidget({ data, isLoaded }: RealProductivityWidgetProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Real Productivity</CardTitle>
        <CardDescription>Productive time vs. total time since 4 AM.</CardDescription>
      </CardHeader>
      <CardContent>
        {isLoaded ? (
          <>
            <div className="text-4xl font-bold text-primary mb-4">{latestProductivity.toFixed(1)}%</div>
            <div className="h-[150px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                  ...
                  />
                  <Line type="monotone" dataKey="productivity" stroke="hsl(var(--primary))" strokeWidth={2} dot={{ r: 4 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </>
