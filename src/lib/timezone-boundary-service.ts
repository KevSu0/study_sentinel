...
export class TimezoneBoundaryService {
  static getStudyDayBoundary(
    ...
    if (rule === 'IST_4AM') {
      return this.getISTBoundary(date);
    } else {
      return this.getUTCBoundary(date);
    }
  }

  private static getISTBoundary(date: Date): BoundaryServiceResult {
  ...
  private static getUTCBoundary(date: Date): BoundaryServiceResult {
