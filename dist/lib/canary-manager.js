...
class CanaryManager {
    constructor() {
        this.devices = [];
        this.storageKey = 'canary_manager_config';
        this.loadConfig();
    ...
    // Public API methods
    isCanaryDevice(deviceId) {
        return this.config.canaryDevices.includes(deviceId);
    }
    isEligibleForRollout(deviceId) {
    ...
    enablePercentageRollout(percentage) {
        this.config.enabled = true;
    ...
    getActiveCanaries() {
        return this.devices.filter(d => this.config.canaryDevices.includes(d.id) && d.status === 'active');
    }
    generateReport() {
...
- Max Error Rate: ${(this.config.criteria.maxErrorRate * 100).toFixed(0)}%
    ...
    canProgressToNextStage() {
        ...
        const criteriaMet = migrationSuccess >= this.config.criteria.minMigrationSuccess &&
...
window.canaryManager = canaryManager;
